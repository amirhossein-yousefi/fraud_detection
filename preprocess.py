...
class DateExtraction(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, date_variables=None):
        self.variables = date_variables
    ...
    def transform(self, X):
        X = X.copy()
        X[self.variables] = pd.to_datetime(X[self.variables])
        X["trans_date"] = X[self.variables].dt.date
        X["trans_date"] = pd.to_datetime(X["trans_date"])

        X['year'] = X['trans_date'].dt.year
        X['month'] = X['trans_date'].dt.month
        X['day'] = X['trans_date'].dt.day
...
class CategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_enc = pd.DataFrame(X_enc,
                             columns=[self.variables[j] + '_' + str(i) for j in range(len(self.variables)) for i in
                                      range(len(self.enc.categories_[j]))])
...
class TargetEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        self.enc.fit(y.values.reshape(-1, 1))
    ...
    def transform(self, X, y):
        ...
        y_enc = self.enc.transform(y.values.reshape(-1, 1)).toarray()
